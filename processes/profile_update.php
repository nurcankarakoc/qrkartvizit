...
function process_social_logo_upload(int $user_id, array $file_array, int $index): ?string
{
    ...
    $upload_dir = '../assets/uploads/social_logos/';
    if (!is_dir($upload_dir) && !mkdir($upload_dir, 0755, true) && !is_dir($upload_dir)) {
        return null;
    }
    ...
    return 'assets/uploads/social_logos/' . $file_name;
}
...
$resolved_cover_color = normalize_hex_color((string)($_POST['cover_color'] ?? '')) ?: '#0A2F2F';
$resolved_text_color = normalize_hex_color((string)($_POST['text_color'] ?? '')) ?: '#FFFFFF';
$brand_color = $resolved_cover_color;
$resolved_avatar_color = $resolved_cover_color;
$default_avatar_initial = derive_initial_from_name($full_name, 'A');
$avatar_initial = $default_avatar_initial;
$brand_palette = normalize_palette_json(json_encode([$brand_color], JSON_UNESCAPED_UNICODE), $brand_color);
$qr_color = normalize_hex_color((string)($_POST['qr_color'] ?? '')) ?? $brand_color;
$qr_bg_color = normalize_hex_color((string)($_POST['qr_bg_color'] ?? '')) ?? '#FFFFFF';
...
$qr_frame_style = normalize_enum((string)($_POST['qr_frame_style'] ?? ''), ['classic', 'soft', 'badge', 'none'], 'soft');
...
$profile_id = (int)($current_profile['id'] ?? 0);
...
$resolved_package_for_access = $package_state['package_slug'] !== ''
    ? $package_state['package_slug']
    : (string)($order['package'] ?? '');
$is_digital_profile_active = qrk_user_has_digital_access($pdo, $user_id, $resolved_package_for_access);
$current_package_definition = (array)($package_state['definition'] ?? qrk_get_package_definition($resolved_package_for_access, $pdo));
$lock_profile_qr_to_print_design = $is_digital_profile_active && (bool)($current_package_definition['has_physical_print'] ?? false);
$has_preview_checkout_gate = $package_state['package_slug'] === ''
    && (string)($package_state['pending_package_slug'] ?? '') !== ''
    && (string)($package_state['pending_package_mode'] ?? '') === 'preview';

if ($has_preview_checkout_gate) {
    header("Location: ../customer/purchase-review.php?status=profile_gate");
    exit();
}

if ($lock_profile_qr_to_print_design) {
    $existing_qr_style = json_decode((string)($current_profile['qr_style'] ?? ''), true);
    if (!is_array($existing_qr_style)) {
        $existing_qr_style = [];
    }

    $qr_color = normalize_hex_color((string)($current_profile['qr_color'] ?? ''))
        ?? normalize_hex_color((string)($existing_qr_style['qr_color'] ?? $existing_qr_style['color'] ?? ''))
        ?? $brand_color;
    $qr_bg_color = normalize_hex_color((string)($current_profile['qr_bg_color'] ?? ''))
        ?? normalize_hex_color((string)($existing_qr_style['qr_bg_color'] ?? $existing_qr_style['bg_color'] ?? ''))
        ?? '#FFFFFF';
    $qr_dot_style = normalize_enum(
        (string)($current_profile['qr_dot_style'] ?? $existing_qr_style['qr_dot_style'] ?? $existing_qr_style['dot_style'] ?? ''),
        ['square', 'dots', 'rounded', 'classy', 'classy-rounded', 'extra-rounded'],
        'square'
    );
    $qr_corner_style = normalize_enum(
        (string)($current_profile['qr_corner_style'] ?? $existing_qr_style['qr_corner_style'] ?? $existing_qr_style['corner_style'] ?? ''),
        ['square', 'dot', 'extra-rounded'],
        'square'
    );
    $qr_frame_style = normalize_enum(
        (string)($current_profile['qr_frame_style'] ?? $existing_qr_style['qr_frame_style'] ?? $existing_qr_style['frame_style'] ?? ''),
        ['classic', 'soft', 'badge', 'none'],
        'classic'
    );
    $qr_style_payload = json_encode([
        'qr_color' => $qr_color,
...
if ($is_digital_profile_active) {
    ...
    if (table_has_column($pdo, 'profiles', 'text_color')) {
        $update_fields[] = 'text_color = ?';
        $update_params[] = $resolved_text_color;
    }
...
if (table_has_column($pdo, 'profiles', 'is_active')) {
    $update_fields[] = 'is_active = ' . ($is_digital_profile_active ? '1' : '0');
}
...
if ($is_digital_profile_active && table_exists($pdo, 'social_links')) {
    if (!table_has_column($pdo, 'social_links', 'logo_path')) {
        try {
            $pdo->exec("ALTER TABLE social_links ADD COLUMN logo_path VARCHAR(255) NULL AFTER url");
        } catch (Throwable $e) {
            // no-op: continue without logo support when DB user has no ALTER privilege
        }
    }
    $supports_social_logo = table_has_column($pdo, 'social_links', 'logo_path');
    ...
    $existing_platform_logos = $_POST['existing_platform_logos'] ?? [];
    $platform_logo_files = $_FILES['platform_logos'] ?? [];
    if (is_array($platforms) && is_array($urls)) {
        if (!is_array($custom_platforms)) {
            $custom_platforms = [];
        }
        if (!is_array($existing_platform_logos)) {
            $existing_platform_logos = [];
        }

        $stmt_insert = $supports_social_logo
            ? $pdo->prepare("INSERT INTO social_links (profile_id, platform, url, logo_path) VALUES (?, ?, ?, ?)")
            : $pdo->prepare("INSERT INTO social_links (profile_id, platform, url) VALUES (?, ?, ?)");
        foreach ($platforms as $index => $platform_raw) {
            ...
            if ($supports_social_logo) {
                $logo_path = null;
                if ($selected_platform === '__custom__') {
                    $uploaded_logo = process_social_logo_upload($user_id, $platform_logo_files, (int)$index);
                    $existing_logo = normalize_existing_social_logo_path((string)($existing_platform_logos[$index] ?? ''));
                    $logo_path = $uploaded_logo ?? $existing_logo;
                }
                $stmt_insert->execute([$profile_id, $platform, $url, $logo_path]);
            } else {
