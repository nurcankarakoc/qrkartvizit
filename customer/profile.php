...
if (!$profile) {
    ...
    $slug = strtolower(str_replace(' ', '-', explode('@', $_SESSION['user_name'])[0])) . '-' . rand(100, 999);
    $stmt = $pdo->prepare("INSERT INTO profiles (user_id, slug, full_name) VALUES (?, ?, ?)");
    $stmt->execute([$user_id, $slug, $_SESSION['user_name']]);
...
<head>
    ...
    <style>
        ...
        .form-control:focus { border-color: var(--primary); background: #fff; outline: none; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
        ...
        .btn-save { background: var(--navy-dark); color: #fff; border: none; padding: 1rem 2.5rem; border-radius: 12px; font-weight: 700; cursor: pointer; transition: 0.3s; }
...
<body>
    ...
    <main class="main-content">
        <header style="margin-bottom: 3rem;">
            <h1 style="font-size: 2rem; font-weight: 800;">Profilimi Düzenle</h1>
            ...
            <div class="card">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="full_name" class="form-control" value="<?php echo $profile['full_name']; ?>" placeholder="Mehmet Yılmaz">
                    </div>
                    <div class="form-group">
                        <label>Mesleki Unvan</label>
                        <input type="text" name="title" class="form-control" value="<?php echo $profile['title']; ?>" placeholder="Yazılım Geliştirici">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="company" class="form-control" value="<?php echo $profile['company']; ?>" placeholder="Zerosoft Teknoloji">
                    </div>
                    <div class="form-group">
                        <label>İş Telefonu</label>
                        <input type="tel" name="phone_work" class="form-control" value="<?php echo $profile['phone_work']; ?>" placeholder="0532 ...">
                    </div>
                </div>

                <div class="form-group">
                    <label>Kısa Biyografi / Hakkında</label>
                    <textarea name="bio" class="form-control" rows="3" placeholder="Örn: 10 yıllık deneyime sahip dijital pazarlama uzmanı..."><?php echo $profile['bio']; ?></textarea>
                </div>

                <h3 style="font-weight: 800; font-size: 1.25rem; margin: 2.5rem 0 1.5rem;">Sosyal Medya & Linkler</h3>
                ...
                <div id="social-links-container">
                    <?php if(empty($links)): ?>
                        <div class="social-link-row">
                            ...
                            <button type="button" class="btn-save" style="background:#f1f5f9; color:#ef4444; padding:0.8rem;" onclick="removeRow(this)">Sil</button>
                        </div>
                    <?php else: ?>
                        <?php foreach($links as $link): ?>
                            <div class="social-link-row">
                                ...
                                <input type="text" name="urls[]" class="form-control" value="<?php echo $link['url']; ?>" placeholder="Linkinizi buraya yapıştırın...">
                                <button type="button" class="btn-save" style="background:#f1f5f9; color:#ef4444; padding:0.8rem;" onclick="removeRow(this)">Sil</button>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                ...
                <button type="button" onclick="addSocialRow()" style="background: rgba(59, 130, 246, 0.1); color: var(--primary); border:none; padding: 0.8rem; border-radius:12px; font-weight:700; cursor:pointer; margin-bottom: 2.5rem;">+ Yeni Link Ekle</button>
        ...
        function previewImage(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    const img = document.querySelector('.image-upload img') || document.createElement('img');
                    img.src = e.target.result;
                    if(!document.querySelector('.image-upload img')) {
